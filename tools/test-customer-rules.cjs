...
(async () => {
  ...
  input.texts=[{text:'ABCDEF',color:'vit'}];input.symbolPlacement='efter';
  assert.equal(r.symbolChoiceAllowed(input,'tass','bada'),false);
  assert.equal(r.symbolChoiceAllowed(input,'ingen'),true);
  input.symbol='ingen';input.texts[0].text='ABCDEFG';
